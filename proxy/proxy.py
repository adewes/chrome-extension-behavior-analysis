...
class Connection(object):
    ...
    def __init__(self, what, request=None, log_queue=None):
        self.buffer = b''
        self.closed = False
    ...
    def recv(self, bytes=8192):
        try:
            ...
            if self.request is not None:
                self.request.data_received+=data
            if len(data) == 0:
...
class Server(Connection):
    ...
    def __init__(self, host, port, request):
        super(Server, self).__init__(b'server', request)
    ...
    def _process_request(self, data):
        ...
        if self.request.state == HTTP_PARSER_STATE_COMPLETE:
            ...
            if self.request.method == b"CONNECT":
                ...
                self.log_queue.put((text_(self.request.method), text_(host), port, None,0))
            elif self.request.url:
                host, port = self.request.url.hostname, self.request.url.port if self.request.url.port else 80
                self.log_queue.put((text_(self.request.method), text_(text_type(host)), port, text_(self.request.url.geturl()),len(self.response.raw)))
            self.server = Server(host, port, self.request)
            try:
...
class TCP(object):
    ...
    def __init__(self, hostname='127.0.0.1', port=8899, backlog=100, log_queue=None):
...
class HTTP(TCP):
    ...
    def handle(self, client):
        proc = Proxy(client, log_queue=self.log_queue)
        proc.daemon = True
        proc.start()
...
class HTTPProcess(multiprocessing.Process):
    ...
    def __init__(self, hostname, port):
        super(HTTPProcess, self).__init__()
        self.hostname = hostname
        self.port = port
    ...
    def run(self):
        self.proxy = HTTP(self.hostname, self.port, log_queue=self.log_queue)
...
def main():
    ...
    proxy = HTTPProcess(hostname, port)
    proxy.start()
    print("Server is running...")
    try:
        while True:
            time.sleep(0.1)
            if not proxy.log_queue.empty():
                print(proxy.log_queue.get(block=False))
    except KeyboardInterrupt:
        print("Quitting...")
        proxy.terminate()
